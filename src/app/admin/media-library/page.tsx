          ...
          {media.map(item => {
            ...
            return (
              <div
                ...
                className={`group relative bg-white rounded-xl border-2 overflow-hidden cursor-pointer transition-all hover:shadow-md ${isSelected ? 'border-[#ff5e14] ring-2 ring-[#ff5e14]/20 shadow-md' : 'border-slate-100 hover:border-slate-200'}`}
              >
                {/* Checkbox */}
                <div className={`absolute top-2 left-2 z-10 w-6 h-6 rounded-md flex items-center justify-center transition-all ${isSelected ? 'bg-[#ff5e14] text-white' : 'bg-white/80 border border-slate-300 opacity-0 group-hover:opacity-100'}`}>
